...
file_put_contents('dannue.txt', var_export($userdata, 1), FILE_APPEND);


// Заголовок письма
$subject = 'Заказ от '.date('d.m.Y').'г.';
// ваш Email
$admin_mail = 'info@dveri-optom.com';
...
<body>
  <p>Информация о заказчике:</p>
	<ul>
		...
		<li><b>Регион:</b> '.$userdata['user_address'].'</li>
		<li><b>Город:</b> '.$userdata['user_comment'].'</li>
		<li><b>Адрес:</b> '.$userdata['user_adress'].'</li>
		<li><b>Способ доставки:</b> '.$userdata['user_name1'].'</li>
		<li><b>Способ оплаты:</b> '.$userdata['user_phone1'].'</li>
	</ul>
...
// Отправка
$send_ok = mail($to, $subject, $message, $body, implode("\r\n", $headers));
// Ответ на запрос
$response = [
	'errors' => !$send_ok,
